...
namespace QuantifyWebAPI.Controllers
{
    public class JobBusinessLogic
    {
        ...
        public string GetJobData(JObject jsonResult)
        {
            ...
            ObjectVersions myObjectVersions = jsonResult.ToObject<ObjectVersions>();
            ...
            foreach (StockingLocationListItem jobsiteItem in all_jobsites)
            {
                StockingLocation jobsite = StockingLocation.GetStockingLocation(jobsiteItem.StockingLocationID, false);
                bool integrateJob;
                if (myObjectVersions.object_version_dict.ContainsKey(jobsite.Number))
                {
                    byte oldVersionStamp = myObjectVersions.object_version_dict[jobsite.Number];
                    //***** For job with differing version stamp from XREF, we know we need to integrate job *****
                    integrateJob = jobsite.VersionStamp[jobsite.VersionStamp.Length] != oldVersionStamp;
                }
                else
                {
                    //***** For job that does not exist yet, we know we need to integrate job *****
                    integrateJob = true;
                }
                    if (integrateJob)
                    {
                        ...
                        //TODO: ADH 9/1/2020 - Identify data locations/mappings for department
                        myJobData.department = "20";
                        //TODO: ADH 9/2/2020 - Identify data location for job type 
                        //(mapping to WebApps codes will be done in Boomi, just need field)
                        myJobData.job_type = "S";

                        //TODO: ADH 9/1/2020 - Identify data location of retention percentage?
